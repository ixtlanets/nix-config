{
  ...
  requiredTotalMiB ? 32768,
  estimatedStackMiB ? 24576,
  minimumFreePercent ? 65,
}:
{
  config,
  lib,
...
let
  ...
  legacyDashboardDbPath = "${dashboardDirectory}/server/llama-dashboard.db";
  # llama-server divides the total context pool between parallel slots.
  totalContextSize = contextSize * parallel;
  ...
  generationPlist = mkLlamaPlist {
    ...
    arguments = [
      ...
      "draft-mtp"
      "--spec-draft-n-max"
      (toString speculativeDraftMax)
      "--cache-ram"
      (toString promptCacheMiB)
      "--n-gpu-layers"
      "all"
      "--metrics"
      "--reasoning"
  ...
  llamaStack = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      legacy_dashboard_db=${lib.escapeShellArg legacyDashboardDbPath}

      required_total_mib=${toString requiredTotalMiB}
      estimated_stack_mib=${toString estimatedStackMiB}
      minimum_free_percent=${toString minimumFreePercent}
      ...
      Usage: llama-stack {preflight|start|stop|restart|status|logs [-f]}
        ...
        start      Build the dashboard and start the full stack
        stop       Stop the dashboard and llama-server
        restart    Rebuild the dashboard and restart the full stack
      ...
      start_service() {
        local name=$1 label=$2 plist=$3 port
        shift 3
        ...
        for port in "$@"; do
          if port_listening "$port"; then
            printf 'llama-stack: port %s is owned by an unmanaged process; stop it before starting %s\n' \
              "$port" "$name" >&2
            return 1
          fi
        done
      ...
      start_servers() {
        start_service llama-server "$generation_label" "$generation_plist" 8080
      }
      ...
      migrate_dashboard_database() {
        if [[ -e "$dashboard_db" || ! -f "$legacy_dashboard_db" || "$dashboard_db" == "$legacy_dashboard_db" ]]; then
          return
        fi

        printf 'Migrating dashboard database to %s\n' "$dashboard_db"
        sqlite3 "$legacy_dashboard_db" ".backup '$dashboard_db'"
      }
      ...
      start_stack() {
        ...
        if ! start_service llama-dashboard "$dashboard_label" "$dashboard_plist" 8081 8082; then
