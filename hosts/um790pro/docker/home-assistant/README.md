...
3. Интеграция frigate: Настройки → Устройства и службы → Добавить → Frigate;
   url `http://127.0.0.1:8971` (интеграция задаётся с точки зрения хоста) и
   long-lived token из frigate (frigate → Settings → API tokens; auth включён).
   Сама интеграция изнутри HA-контейнера ходит на `http://frigate:5000`
   (общая сеть `hamqtt`) — оба адреса рабочие.
