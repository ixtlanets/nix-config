...
4. Интеграция frigate: Добавить → Frigate;
   - URL: `https://192.168.1.241:8971` (frigate биндит порт на LAN IP, не на
     loopback; auth включён → TLS). `validate_ssl` — выключить
     (самоподписанный сертификат).
   - Username/password: пользователь frigate (admin). Токены интеграция
     не спрашивает — она сама ходит в логин frigate.
   - Внутри HA (host network) всё остальное доступно так же, как с хоста:
     RTSP-рестрим `192.168.1.241:8554`, MQTT — через брокера выше.
