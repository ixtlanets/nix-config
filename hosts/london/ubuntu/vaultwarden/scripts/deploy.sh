...
for secret in "$SECRET_DIR/vaultwarden.env" "$SECRET_DIR/backup.env"; do
  if head -c 10 "$secret" | LC_ALL=C grep -aq 'GITCRYPT'; then
    echo "secret is still encrypted; run git-crypt unlock before deploy: $secret" >&2
    exit 1
  fi
done
...
cleanup() {
  # tmp_remote is generated by mktemp on the remote host.
...
# Values are controlled local configuration; tmp_remote is generated remotely.
# shellcheck disable=SC2029
...
sudo docker run --rm \
  -v "$TMP_REMOTE/Caddyfile:/etc/caddy/Caddyfile:ro" \
  caddy:2.11.4 \
  caddy validate --config /etc/caddy/Caddyfile >/dev/null

# Parse the staged env files before replacing the live copies. This keeps a
# locked git-crypt checkout or malformed env file from taking down the service
# during systemd's stop-before-start restart sequence.
sudo docker compose \
  -f "$TMP_REMOTE/docker-compose.yml" \
  config >/dev/null
