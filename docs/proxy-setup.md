...
### UM790 Pro subnet router

`um790pro` routes two LAN-only Macs into the tailnet while Tailscale remains stopped on the Macs:

| Endpoint | Reserved LAN route |
|----------|--------------------|
| M1 Max | `192.168.1.174/32` |
| M3 Max | `192.168.1.144/32` |

Keep both addresses reserved in the home router's DHCP configuration. The active CachyOS install
on `um790pro` owns the routes; its NixOS host configuration declares the equivalent server role.
The router advertises only these two `/32`s, rejects routes advertised by other nodes, and leaves
Tailscale's default subnet SNAT enabled. The Macs therefore see the current LAN address of
`um790pro` as the connection source (`192.168.1.241` was observed during setup), so that address
must be discovered rather than hardcoded in firewall or packet-capture commands.

Tailnet policy grants member-owned, tagged, and shared Tailscale devices all IP protocols and ports
to the two host aliases. Destination services and the macOS firewall still decide what answers.
Linux clients must enable `--accept-routes=true`; managed `zenbook` and `x1carbon` declare that
preference. Other desktop and mobile clients normally accept subnet routes by default. Access is by
IP only: MagicDNS does not create names for Macs whose own Tailscale clients are stopped. A sleeping,
disconnected, or absent Mac is expected to time out while its advertised route remains present.

On the active CachyOS installation, converge the router settings with:

```bash
./install.sh tailscale-subnet-router
```

The focused action requires an active, authenticated `tailscaled`, keeps global IPv6 forwarding
disabled, enables IPv4 forwarding through `/etc/sysctl.d/99-tailscale-subnet-router.conf`, and
applies the two routes with SNAT. The normal full installer invokes the same helper in best-effort
mode and prints the login-and-rerun instruction if the machine has not joined the tailnet yet.

The Tailscale admin policy must preserve its existing rules while defining stable host aliases for
`192.168.1.174` and `192.168.1.144`, and a dedicated grant from `autogroup:member`,
`autogroup:tagged`, and `autogroup:shared` to both aliases with `ip: ["*"]`. Approve each `/32`
manually; never substitute `192.168.1.0/24` or a `"*"` source selector.

Rollback keeps `um790pro` online:

1. Withdraw advertisements on `um790pro` with `sudo tailscale set --advertise-routes=`.
2. Disable both `/32` routes in the Tailscale admin console.
3. Remove or disable the dedicated grant if access must be revoked.
4. Revert the advertisements in both `install.sh` and the NixOS `um790pro` configuration.
5. Leave Tailscale running so `um790pro` remains remotely reachable.
6. Disable `--accept-routes` on a client only if it should stop consuming every tailnet subnet route.

Do not use London for a rollback drill. Its only permitted subnet-router change is a one-time,
route-conflict-audited `--accept-routes=true`, followed by health checks without restarting or
changing microsocks, Tailscale Serve, Caddy, or the proxy bundle.
